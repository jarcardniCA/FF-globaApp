...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Lo siento, parece que este usuario no existe o la cuenta ha sido eliminada.')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Error en los datos proporcionados. ')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text(
              'Se ha enviado un enlace de restablecimiento de contraseña a tu dirección de correo electrónico')),
    );
  }
    ...
    phoneAuthManager.addListener(() {
      ...
      if (phoneAuthManager.triggerOnCodeSent) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Error en los datos proporcionados. '),
        ));
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : GlobaAppFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' =>
          'El correo electrónico proporcionado está relacionado a una cuenta existente. Utilice otro disponible.',
        'INVALID_LOGIN_CREDENTIALS' =>
          'Las credenciales ingresadas no son válidas. Por favor, inténtalo de nuevo.',
        _ => 'Error en los datos proporcionados. ',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
